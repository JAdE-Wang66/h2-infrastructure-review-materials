function [final_result,final_result2,final_result3] = multiobj_best_sce1()
...
D_list(:,:,5) = xlsread('D:\Desktop\氢能重卡\爬虫\最终结果\2029年\步骤3 - 计算网格化需求中心点与加油站距离.xlsx',...
    'C2:KM321');
D_list(:,:,6) = xlsread('D:\Desktop\氢能重卡\爬虫\最终结果\2029年\步骤3 - 计算网格化需求中心点与加油站距离.xlsx',...
    'C2:KM321');
%需求列向量,单位为Kg
...
Xvqiu_xiaoqu_list = Xvqiu_xiaoqu_list * 1.05; %最差情景
...
min_obj1 = 88030.52099;
max_obj1 = 1247688.544;
min_obj2 = 19578.64935;
max_obj2 = 600169.9021;
min_obj3 = 74462.53511;
max_obj3 = 74462.53511;
...
% sub_max_mid=200;%中型加氢站的一次性补贴金额
% sub_max_large=240;%大型加氢站的一次性补贴金额
% C = [];
% %% 各单目标最优值确定
% for i = 1:len
%     D = D_list(:,:,i);
...
%     C=[C,subsidy_large_temp>=0];%补贴大于等于0
%     C=[C,subsidy_mid_temp<=(on_mid_temp-on_mid_temp_)*sub_max_mid];%按一次性补贴
%     C=[C,subsidy_large_temp<=(on_large_temp-on_large_temp_)*sub_max_large];
...
BH_large_temp_ = 0;%前一年的大型油氢合建站装机容量
on_large_temp_ = 0;
b_up = 8000;%中大型加氢站的容量上限
b_mid = 1000;%小型加氢站的容量上限；中大型加氢站的容量下限
b_low = 500;%小型加氢站的容量下限
sub_max_mid=200;%小型加氢站的一次性补贴金额
sub_max_large=500;%中大型加氢站的一次性补贴金额
C = [];
%% 各单目标最优值确定
for i = 1:len
    D = D_list(:,:,i);
...
obj_temp = 100*( w1 * ( OBJ1 - max_obj1 ) / ( max_obj1 - min_obj1 ) + ...
    w2 * ( OBJ2 - max_obj2 ) / ( max_obj2 - min_obj2 ) + ...
    w3 * ( min_obj3 - OBJ3 ) / ( max_obj3 - min_obj3 ));
%年间只允许扩建约束
BH = BH_mid + BH_large;
C=[C,BH<=b_up];
for i = 2:len
...
save('multiobj_beijingxianyouzhengce_best')
end
